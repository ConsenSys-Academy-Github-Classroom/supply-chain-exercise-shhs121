...
contract SupplyChain {
  ...
  event LogSold(uint sku, address buyer, address seller, uint price);
  // <LogShipped event: sku arg>
  event LogShipped(uint sku);
  // <LogReceived event: sku arg>
  event LogReceived(uint sku);

  // event Tester(State state);
  ...
  modifier checkValue(uint _sku) {
    ...
    (bool success, bytes memory data) =  items[_sku].buyer.call{value: amountToRefund}("This is a refund of extra funds transferred by buyer.");
    require(success);
  }
  ...
  function buyItem(uint _sku) public payable forSale(_sku) paidEnough(items[_sku].price) checkValue(_sku) {
    ...
    items[_sku].buyer = payable(msg.sender);
     ...
     require(sent, "Error in call");

    items[_sku].state = State.Sold;
    
    emit LogSold(_sku, items[_sku].buyer, items[_sku].seller, items[_sku].price);
  }
  ...
  function fetchItem(uint _sku) public view returns (string memory name, uint sku, uint price, State state, address seller, address buyer)  
   { 
    ...
    //  emit Tester(state); //Inserted by me
